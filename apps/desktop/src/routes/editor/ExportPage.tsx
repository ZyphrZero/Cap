...
export const COMPRESSION_OPTIONS: Array<{
	...
	{
		get label() {
			return t("editor.export.compression.potato");
		},
		value: "Potato",
		bpp: 0.04,
	},
	{
		get label() {
			return t("editor.export.compression.web");
		},
		value: "Web",
		bpp: 0.08,
	},
	{
		get label() {
			return t("editor.export.compression.social");
		},
		value: "Social",
		bpp: 0.15,
	},
	{
		get label() {
			return t("editor.export.quality.maximum");
		},
		value: "Maximum",
		bpp: 0.3,
	},
...
export const EXPORT_TO_OPTIONS = [
	{
		get label() {
			return t("editor.export.exportTo.file.label");
		},
		value: "file",
		icon: IconCapFile,
		get description() {
			return t("editor.export.exportTo.file.description");
		},
	},
	{
		get label() {
			return t("editor.export.exportTo.clipboard.label");
		},
		value: "clipboard",
		icon: IconCapCopy,
		get description() {
			return t("editor.export.exportTo.clipboard.description");
		},
	},
	{
		get label() {
			return t("editor.export.exportTo.link.label");
		},
		value: "link",
		icon: IconCapLink,
		get description() {
			return t("editor.export.exportTo.link.description");
		},
	},
] as const;
...
export function ExportPage() {
	...
	const exportedAssetLabel = () =>
		isMovCursorOnlyExport()
			? t("editor.export.assets.cursorTrack")
			: settings.format === "Gif"
				? "GIF"
				: t("editor.export.assets.recording");
	const exportMediumLabel = () =>
		isMovCursorOnlyExport()
			? t("editor.export.assets.cursorTrackLower")
			: settings.format === "Gif"
				? "GIF"
				: t("editor.export.assets.video");

	const handleCancel = async () => {
		if (
			await ask(t("editor.export.messages.cancelConfirm"), {
				title: t("editor.export.messages.cancelTitle"),
				kind: "warning",
			})
		) {
			setIsCancelled(true);
	...
	const copy = createMutation(() => ({
		...
		onError: (error) => {
			...
			commands.globalMessageDialog(
				error instanceof Error
					? error.message
					: t("editor.export.messages.errorCopying"),
			);
			setExportState(reconcile({ type: "idle" }));
		},
		onSuccess() {
	...
	const save = createMutation(() => ({
		...
		onError: (error) => {
			...
			commands.globalMessageDialog(
				error instanceof Error
					? error.message
					: t("editor.export.messages.errorExporting", {
							error: String(error),
						}),
			);
			setExportState({ type: "idle" });
		...
		onError: (error) => {
			...
			if (!(error instanceof SilentError)) {
				commands.globalMessageDialog(
					error instanceof Error
						? error.message
						: t("editor.export.messages.uploadFailed"),
				);
			}
			...
			<div class="flex-1 min-h-0 flex relative">
				<div class="flex-1 min-h-0 p-5 flex flex-col">
					<div class="flex items-center gap-1.5 mb-2">
						...
						<Tooltip content={t("editor.export.previewNote")}>
					...
					<div class="flex-1 overflow-y-auto p-4 space-y-5">
								...
								<For each={EXPORT_TO_OPTIONS}>
									{(option) => {
										...
										const disabledReason = () =>
											isDisabled()
												? cursorOnly()
													? t("editor.export.cursorOnlyLinkDisabled")
													: t("editor.export.transparentLinkDisabled")
												: undefined;
										const button = (
											<button
												type="button"
							...
							<div class="flex gap-1.5">
								<For each={FORMAT_OPTIONS}>
									{(option) => {
										...
										const disabledReason = () =>
											cursorOnly()
												? t("editor.export.cursorOnlyFormatDisabled")
												: option.value === "Mp4" && requiresTransparentExport()
						...
						<Show when={settings.format === "Mp4" && !cursorOnly()}>
							...
							>
								<div class="grid grid-cols-4 gap-1.5">
									<For each={[...COMPRESSION_OPTIONS].reverse()}>
										{(option) => (
											...
											>
												{option.label === t("editor.export.compression.social")
													? t("editor.export.compression.socialShort")
													: option.label}
											</button>
										)}
									</For>
								...
								>
									...
									<div class="text-left">
										<span class="block">
											{t("editor.export.optimizeFileSize")}
										</span>
						...
						>
							...
							>
								<span>
									{advancedMode()
										? t("editor.export.hideOptions")
										: t("editor.export.showOptions")}
								</span>
								<IconCapChevronDown
									class={cx(
							...
							<Show when={advancedMode()}>
								<div class="mt-3 space-y-4">
									...
									>
										...
										<div class="text-left">
											<span class="block">
												{t("editor.export.exportCursorOnly")}
											</span>
											<span class="text-[10px] text-gray-9">
											...
											<div class="flex items-start gap-2">
												...
												<div class="text-left">
													...
													<p class="text-[10px] text-amber-11">
														{t("editor.export.cursorOnlyWarning")}
													</p>
												</div>
											</div>
										</div>
									...
									<Show when={settings.format === "Mp4" && !cursorOnly()}>
										<div class="space-y-2 border-t border-gray-4 pt-3">
											...
											<div class="flex justify-between text-[10px] text-gray-9">
												<span>{t("editor.export.tinyBitrate")}</span>
												<span>{t("editor.export.hugeBitrate")}</span>
											</div>
												...
												<div class="mt-4 pt-3 border-t border-gray-4">
													...
													>
														...
														<div class="text-left">
															...
															<span class="text-[10px] text-gray-9">
																{t(
																	"editor.export.forceFfmpegDecoderDescription",
																)}
															</span>
														</div>
													</button>
				...
				<div class="p-4">
					<div class="flex items-center justify-between mb-4">
						<h2 class="text-gray-12 font-medium">
							{t("editor.export.qualityPreview")}
						</h2>
						<button
							type="button"
					...
					<div class="relative aspect-video rounded-lg overflow-hidden bg-gray-4 flex items-center justify-center">
						<Show when={previewUrl()}>
							{(url) => (
								<img
									src={url()}
									alt={t("editor.export.previewFullAlt")}
					...
					<div class="flex justify-between text-sm text-gray-11 mt-4">
						...
						<Show when={renderEstimate()}>
							{(est) => {
								return (
									<span>
										{t("editor.export.estimatedSizeWithValue", {
											size: est().estimatedSizeMb.toFixed(1),
										})}
									</span>
								);
							}}
									...
									{(copyState) => (
										<Switch>
														...
														heading={
															renderState.type === "rendering"
																? t("editor.export.status.rendering", {
																		type: exportMediumLabel(),
																	})
																: t("editor.export.status.preparingExport")
														}
														state={renderState}
											...
											<Match when={copyState.type === "copying"}>
												<ActiveExport
													heading={t("editor.export.status.copying")}
												/>
											</Match>
											<Match when={copyState.type === "done"}>
												<CompletedExport
													title={t("editor.export.status.copied")}
													subtitle={t("editor.export.status.readyToPaste", {
														type: exportMediumLabel(),
													})}
												/>
											</Match>
									...
									{(saveState) => (
										<Switch>
											...
											<Match when={saveState.type === "copying"}>
												<ActiveExport
													heading={t("editor.export.status.savingToFile")}
												/>
											</Match>
											<Match when={saveState.type === "done"}>
												<CompletedExport
													title={t("editor.export.status.exportComplete")}
													subtitle={t("editor.export.status.ready", {
														type: exportMediumLabel(),
													})}
												/>
											</Match>
									...
									{(uploadState) => (
										<Switch>
											...
											>
												{(uploading) => (
													<ActiveExport
														heading={t("editor.export.status.uploading")}
											...
											<Match when={uploadState.type === "done"}>
												<CompletedExport
													title={t("editor.export.status.uploadComplete")}
													subtitle={t("editor.export.status.uploadSuccess")}
												/>
											</Match>
										</Switch>
									)}
									...
									<Show when={exportState.action === "save"}>
										<div class="flex gap-3">
											...
											>
												...
												{t("editor.export.actions.openFile")}
											</Button>
											<Button
												variant="dark"
												class="flex gap-2 items-center"
											...
											>
												...
												{t("editor.export.actions.copyToClipboard")}
											</Button>
										</div>
									</Show>
							...
							<Show when={exportState.type !== "done"}>
								<p class="max-w-sm text-xs leading-relaxed text-center text-gray-11">
									<span class="font-semibold text-gray-12">
										{t("editor.export.tipLabel")}
									</span>{" "}
									{t("editor.export.instantModeTip")}
								</p>
							</Show>
						</div>
					);
				...
				<Show when={frames()}>
					{(rendered) => (
						<p class="text-sm tabular-nums text-gray-11">
							...
							{rendered().totalFrames.toLocaleString()}{" "}
							{t("editor.export.frames")}
						</p>
					)}
				</Show>
			</div>
